...
class AutoTransform extends Transform {
    ...
    AutoTransform(AutoTransformHelper transformHelper) {
    ...
    private void checkRNState() {
        try {
            ...
            try {
                ...
                transformHelper.rnState = AutoTransformHelper.RN_STATE.HAS_VERSION
            } catch (Exception e) {
                transformHelper.rnState = AutoTransformHelper.RN_STATE.NO_VERSION
            }
        } catch (Exception e) {
            transformHelper.rnState = AutoTransformHelper.RN_STATE.NOT_FOUND
        }
    }
            ...
            for (Map.Entry<File, Status> changedFile : fileStatusMap.entrySet()) {
                ...
                switch (status) {
                    case Status.NOTCHANGED:
                        break
                    case Status.REMOVED:
                        InjectLogger.info("目录 status = $status:$inputFile.absolutePath")
                        if (destFile.exists()) {
    ...
    void forEachJar(boolean isIncremental, JarInput jarInput, TransformOutputProvider outputProvider, Context context) {
        ...
        if (isIncremental) {
            ...
            switch (status) {
                ...
                case Status.ADDED:
                case Status.CHANGED:
                    InjectLogger.info("jar status = $status:$destFile.absolutePath")
    ...
    void transformJar(File dest, JarInput jarInput, Context context) {
        ...
        if (!transformHelper.extension.disableJar || jarInput.file.absolutePath.contains('SensorsAnalyticsSDK')) {
            InjectLogger.info("开始遍历 jar：" + jarInput.file.absolutePath)
            modifiedJar = modifyJarFile(jarInput.file, context.getTemporaryDir())
            InjectLogger.info("结束遍历 jar：" + jarInput.file.absolutePath)
        }
        if (modifiedJar == null) {
        ...
        while (enumeration.hasMoreElements()) {
            ...
            if (entryName.endsWith(".DSA") || entryName.endsWith(".SF")) {
                ...
                try {
                    ...
                    sourceClassBytes = AutoAnalyticsUtil.toByteArrayAndAutoCloseStream(inputStream)
                } catch (Exception e) {
                    InjectLogger.error("Exception encountered while processing jar: " + jarFile.getAbsolutePath())
                ...
                if (!jarEntry.isDirectory() && entryName.endsWith(".class")) {
                    ...
                    AutoMatchUtils classNameAnalytics = transformHelper.analytics(className)
     ...
     * 真正修改类中方法字节码
     */
    private byte[] modifyClass(byte[] srcClass, AutoMatchUtils classNameAnalytics) {
        try {
            ClassWriter classWriter = new ClassWriter(ClassWriter.COMPUTE_MAXS)
            ClassVisitor classVisitor = new AutoClassVisitor(classWriter, classNameAnalytics, transformHelper)
            ClassReader cr = new ClassReader(srcClass)
            cr.accept(classVisitor, ClassReader.EXPAND_FRAMES + ClassReader.SKIP_FRAMES)
            return classWriter.toByteArray()
        } catch (Exception ex) {
            InjectLogger.error("$classNameAnalytics.className 类执行 modifyClass 方法出现异常")
            ex.printStackTrace()
    ...
    private File modifyClassFile(File dir, File classFile, File tempDir) {
        ...
        try {
            ...
            AutoMatchUtils classNameAnalytics = transformHelper.analytics(className)
            if (classNameAnalytics.isShouldModify) {
                byte[] sourceClassBytes = AutoAnalyticsUtil.toByteArrayAndAutoCloseStream(new FileInputStream(classFile))
