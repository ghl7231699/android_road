 ...
 */
class AutoWebViewMethodVisitor extends AdviceAdapter implements Opcodes {
    ...
    AutoWebViewMethodVisitor(MethodVisitor mv, int access, String name, String desc, AutoTransformHelper transformHelper, String className, String superName) {
        super(AutoAnalyticsUtil.ASM_VERSION, mv, access, name, desc)
        ...
        try {
            if (opcode != INVOKESTATIC && TARGET_NAME_DESC.contains(name + desc)) {
                ...
                if (isAssignableWebView(owner)) {
                    ...
                    mv.visitMethodInsn(INVOKESTATIC, MethodHookConfig.SENSORS_ANALYTICS_API, name + "2", desc, false)
                    return
                }
            }
        } catch (Throwable throwable) {
            InjectLogger.warn("Can not auto handle webview, if you have any questions, please contact our technical services: classname:${className}, method:${methodNameDesc}, exception: ${throwable}")
        }
        super.visitMethodInsn(opcode, owner, name, desc, itf)
    }
    ...
    private boolean isAssignableWebView(String owner) {
        try {
            ...
            InjectLogger.warn("Can not load class, if you have any questions, please contact our technical services: classname:${className}, exception: ${throwable}")
        }
        return false
    }
