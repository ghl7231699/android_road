...
//        }
    ...
    /**
     * 该方法是当扫描器完成类扫描时才会调用，如果想在类中追加某些方法，可以在该方法中实现。 可选择在return 之前调用
     */
    @Override
    void visitEnd() {
...
//            }
//        }
//        if (LogAnalyticsUtil.isLauncherActivity(mSuperName)) {//如果是启动activity
        if ("com/mmc/lamandys/liba_datapick/activity/MainActivity" == mClassName) {
            Logger.info("||visitEnd:" + mSuperName + mClassName)
            MethodVisitor mv
            ...
            while (iterator.hasNext()) {
                ...
                String key = entry.getKey()
                LogMethodCell methodCell = entry.getValue()
                if (visitedActivityMethods.contains(key)) {StatusBarUtils.java
                    continue
                }
                Logger.info("||Hooked class:injected method:" + methodCell.agentName)
                mv = classVisitor.visitMethod(Opcodes.ACC_PUBLIC, methodCell.name, methodCell.desc, null, null)
