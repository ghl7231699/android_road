...
class AutoDefaultMethodVisitor extends AdviceAdapter {

    String methodName

    AutoDefaultMethodVisitor(MethodVisitor mv, int access, String name, String desc) {
        super(AutoAnalyticsUtil.ASM_VERSION, mv, access, name, desc)
        methodName = name
        if (InjectLogger.debug) {
            InjectLogger.info("开始扫描方法：${InjectLogger.accCode2String(access)} ${methodName}${desc}")
        }
    }
    ...
    void visitEnd() {
        if (InjectLogger.debug) {
            InjectLogger.info("结束扫描方法：${methodName}\n")
        }
        super.visitEnd()
    }
